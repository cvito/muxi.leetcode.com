package source550

import (
	"fmt"
	"math"
)

/**
地址：https://leetcode-cn.com/problems/diameter-of-binary-tree/
题目描述：
给定一棵二叉树，你需要计算它的直径长度。一棵二叉树的直径长度是任意两个结点路径长度中的最大值。这条路径可能穿过根结点。


Example:
给定二叉树

          1
         / \
        2   3
       / \
      4   5
返回 3, 它的长度是路径 [4,2,1,3] 或者 [5,2,1,3]。

注意：两结点之间的路径长度是以它们之间边的数目表示。

 */


/**
 * Definition for a binary tree node.
 * type TreeNode struct {
 *     Val int
 *     Left *TreeNode
 *     Right *TreeNode
 * }
 */

 /**
 思路：

  */

  /**
  用时：4ms
  内存：4.5MB
   */
func diameterOfBinaryTree(root *TreeNode) int {
	if root == nil {
		return 0
	}
	maxPath := new(int)
	depthTree(root, maxPath)
	return *maxPath
}

func depthTree(root *TreeNode, maxPath *int) int {
	if root == nil {
		return 0
	}
	leftPath := depthTree(root.Left, maxPath)
	rightPath := depthTree(root.Right, maxPath)
	fmt.Println("root.val:", root.Val)
	fmt.Println("leftPath:", leftPath)
	fmt.Println("rightPath:", rightPath)
	tmpMax := leftPath + rightPath + 1
	if tmpMax > *maxPath {
		*maxPath = tmpMax
	}
	return int(math.Max(float64(leftPath), float64(rightPath))) + 1
}